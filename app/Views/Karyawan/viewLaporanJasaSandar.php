...
<body class="hold-transition sidebar-mini">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                	<div style="padding-bottom: 20px;">
                                        <form method="post" action="<?= base_url('') ?>/Karyawan/LaporanJasaSandar/data_cetak">
    				                		<div class="row">
    				                			<div class="col-md-5">
    ...
    <script>
        function ganti(status) {
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Karyawan/LaporanJasaSandar/data/' + $('#tanggal').val() + '/' + status).load();
        };
        ...
        $("#input_kapal").select2({
            ...
            theme: 'bootstrap4',
            ajax: {
                url: '<?php echo base_url('Karyawan/JasaSandar/data_kapal'); ?>',
                type: "post",
                delay: 250,
                dataType: 'json',
	    ...
	    $(function() {
	        ...
	        $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
	            ...
	            $('.table').DataTable().ajax.url('<?= base_url() ?>/Karyawan/LaporanJasaSandar/data/' + tanggal + '/' + $('#input_kapal').val()).load();
	        });

            $("#btn_reset").click(function (e) {
                document.getElementById("input_kapal").selectedIndex = 0;
                $('.table').DataTable().ajax.url('<?= base_url() ?>/Karyawan/LaporanJasaSandar/data/' + $('#tanggal').val() + '/' + $('#input_kapal').val()).load();
            });
	        ...
	        $(".table").DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url() ?>/Karyawan/LaporanJasaSandar/data/" + $('#tanggal').val() + '/' + $('#input_kapal').val(),
                    "dataSrc": ""
                },
                "columns": [{
                        "data": "namakp"
